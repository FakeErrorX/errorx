...
function clashMetaAlpha() {
  ...
  return {
    name: "errorx-mihomo-alpha",
    targetFile: `errorx-mihomo-alpha-${SIDECAR_HOST}${isWin ? ".exe" : ""}`,
    exeFile,
    zipFile,
    downloadURL,
  };
...
function clashMeta() {
  ...
  return {
    name: "errorx-mihomo",
    targetFile: `errorx-mihomo-${SIDECAR_HOST}${isWin ? ".exe" : ""}`,
    exeFile,
    zipFile,
    downloadURL,
  };
 ...
 * main
 */
const SERVICE_URL = `https://github.com/FakeErrorX/errorx-service/releases/download/${SIDECAR_HOST}`;

const resolveService = () => {
  let ext = platform === "win32" ? ".exe" : "";
  let suffix = platform === "linux" ? "-" + SIDECAR_HOST : "";
  resolveResource({
    file: "errorx-service" + suffix + ext,
    downloadURL: `${SERVICE_URL}/errorx-service${ext}`,
  });
};
