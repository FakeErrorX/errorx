...
!macro CheckAllVergeProcesses
  ; Check if errorx-service.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "errorx-service.exe"
  !else
    nsis_tauri_utils::FindProcess "errorx-service.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill errorx-service.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "errorx-service.exe"
    !else
      nsis_tauri_utils::KillProcess "errorx-service.exe"
    !endif
  ${EndIf}

  ; Check if errorx-mihomo-alpha.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "errorx-mihomo-alpha.exe"
  !else
    nsis_tauri_utils::FindProcess "errorx-mihomo-alpha.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill errorx-mihomo-alpha.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "errorx-mihomo-alpha.exe"
    !else
      nsis_tauri_utils::KillProcess "errorx-mihomo-alpha.exe"
    !endif
  ${EndIf}

  ; Check if errorx-mihomo.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "errorx-mihomo.exe"
  !else
    nsis_tauri_utils::FindProcess "errorx-mihomo.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill errorx-mihomo.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "errorx-mihomo.exe"
    !else
      nsis_tauri_utils::KillProcess "errorx-mihomo.exe"
    !endif
  ${EndIf}
...
!macro StartVergeService
  ...
  ${If} $0 == 0
    ...
    ${If} $0 == 0
      Push $0
      ${If} $1 == 0
            DetailPrint "Restart ErrorX Service..."
...
!macro RemoveVergeService
  ...
  ${If} $0 == 0
    ...
    ${If} $0 == 0
      Push $0
      ${If} $1 == 1
        DetailPrint "Stop ErrorX Service..."
        SimpleSC::StopService "clash_verge_service" 1 30
        Pop $0 ; returns an errorcode (<>0) otherwise success (0)
        ${If} $0 == 0
              DetailPrint "Removing ErrorX Service..."
              SimpleSC::RemoveService "clash_verge_service"
        ${ElseIf} $0 != 0
                  Push $0
                  SimpleSC::GetErrorMessage
                  Pop $0
                  MessageBox MB_OK|MB_ICONSTOP "ErrorX Service Stop Error ($0)"
        ${EndIf}
  ${ElseIf} $1 == 0
        DetailPrint "Removing ErrorX Service..."
        SimpleSC::RemoveService "clash_verge_service"
...
Section Install
  ...
  SetRegView 64  
  ; 清理旧版本的注册表项 (ErrorX)
  ReadRegStr $R2 HKCU "$R1" "ErrorX"
  ${If} $R2 != ""
    DeleteRegValue HKCU "$R1" "ErrorX"
  ${EndIf}
  
  ReadRegStr $R2 HKLM "$R1" "ErrorX"
  ${If} $R2 != ""
    DeleteRegValue HKLM "$R1" "ErrorX"
  ${EndIf}
  ...
  IfFileExists "$INSTDIR\ErrorX.exe" 0 +2
    Delete "$INSTDIR\ErrorX.exe"
