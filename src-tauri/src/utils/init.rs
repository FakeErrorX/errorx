...
fn init_dns_config() -> Result<()> {
    ...
    if !dns_path.exists() {
        ...
        help::save_yaml(&dns_path, &default_dns_config, Some("# ErrorX DNS Config"))?;
    }
...
pub fn init_config() -> Result<()> {
    ...
    crate::log_err!(dirs::clash_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IClashTemp::template().0, Some("# ErrorXasu"))?;
        }
        <Result<()>>::Ok(())
    }));
    ...
    crate::log_err!(dirs::verge_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IVerge::template(), Some("# ErrorX"))?;
        }
        <Result<()>>::Ok(())
    }));
    ...
    crate::log_err!(dirs::profiles_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IProfiles::template(), Some("# ErrorX"))?;
        }
        <Result<()>>::Ok(())
    }));
...
pub fn init_scheme() -> Result<()> {
    ...
    clash.set_value("", &"ErrorX")?;
    clash.set_value("URL Protocol", &"ErrorX URL Scheme Protocol")?;
