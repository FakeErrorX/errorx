...
export interface LicenseInfo {
  ...
  windows_device: DeviceInfo;
  android_device: DeviceInfo | null;
}
...
export interface ServerInfo {
  ...
  live_servers: number;
  reserved_servers: number;
}

// License validation
export const validateLicense = async (key: string): Promise<boolean> => {
  try {
    return await invoke<boolean>("validate_license", { key });
  } catch (error) {
...
export const getServerList = async (): Promise<ServerInfo> => {
  try {
    return await invoke<ServerInfo>("get_server_list");
  } catch (error) {
...
// Start proxy servers
export const startProxy = async (): Promise<void> => {
  try {
...
// Get proxy status
export const getProxyStatus = async (): Promise<Array<[number, boolean]>> => {
  try {
    return await invoke<Array<[number, boolean]>>("get_proxy_status");
  } catch (error) {
