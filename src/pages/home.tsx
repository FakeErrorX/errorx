...
import { openWebUrl, patchVergeConfig, getVergeConfig } from "@/services/cmds";
import { TestCard } from "@/components/home/test-card";
import { IpInfoCard } from "@/components/home/ip-info-card";
import { LicenseInfoCard } from "@/components/home/license-info-card";
import { ProxyServerCard } from "@/components/home/proxy-server-card";
import { stopProxy, getProxyStatus } from "@/services/api";
      ...
      <DialogContent>
        <FormGroup>
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={cards.license || false}
                onChange={() => handleToggle("license")}
              />
            }
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={cards.proxyserver || false}
                onChange={() => handleToggle("proxyserver")}
              />
            }
...
const HomePage = () => {
  ...
  const toGithubDoc = useLockFn(() => {
    return openWebUrl("https://errorx.me");
  });
  ...
  const handleLogout = useLockFn(async () => {
    try {
      // Get current verge config to check system proxy and tun mode status
      const config = await getVergeConfig();
      
      // Disable system proxy if enabled
      if (config.enable_system_proxy) {
        await patchVergeConfig({ enable_system_proxy: false });
        Notice.success("System proxy disabled");
      }

      // Disable tun mode if enabled
      if (config.enable_tun_mode) {
        await patchVergeConfig({ enable_tun_mode: false });
        Notice.success("Tun mode disabled");
      }
      ...
      if (isRunning) {
        await stopProxy();
        Notice.success("Server stopped successfully");
      }
      ...
      Notice.error("Failed to stop services. Please try again.");
    }
  });
